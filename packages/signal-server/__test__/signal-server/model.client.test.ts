import {
  ServerRunner, IDiff, IHookContext,
  IQueryWhere, debuggerLog,
  IModelData, setGlobalModelEvent, ModelEvent
} from '../../src/index'
...
describe('client model', () => {
  ...
  beforeEach(async () => {
    ...
    mockBM.initModelConfig({
      ...
      async postComputeToServer(c: IHookContext) {
        ...
        const serverRunner = new ServerRunner(mockBM[c.name as keyof typeof mockBM])
      ...
      async postQueryToServer(c: IHookContext): Promise<IHookContext> {
        ...
        const runner = new ServerRunner(mockBM[c.name as keyof typeof mockBM])
  ...
  describe('mount model', () => {
    ...
    it('post query to server', async () => {
      ...
      const runner = new ServerRunner(mockBM.userModelClient)
    ...
    it('keep active model in realtime', async () => {
      ...
      const runner1 = new ServerRunner(mockBM.writeModelWithSource)
      const result1 = runner1.init()

      const runner2 = new ServerRunner(mockBM.writeModelWithSource)
