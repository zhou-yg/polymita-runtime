...
import { equalFileContent, loadJSON, logFrame, statusFrame, traverseFirstDir, tryMkdir, tryUnlinkSync } from "../util";
...
import { IEnhancement, initBlankSchemaPart } from "./prebuild";
...
function getSourceReferenceType (source: IParsedSchemaStruct, targetProp: string) {
  let type = ''
  ...
  if (!type) {
    throw new Error(`[getSourceReferenceType] can not find ${targetProp} in source(name=${source.name})`)
  }
        ...
        case '1:1':
          {
            ...
            const type = getSourceReferenceType(source, 'id')
        ...
        case 'n:n':
          {
            ...
            const sourceReferType = getSourceReferenceType(source, 'id')
            const targetReferType = getSourceReferenceType(target, 'id')
...
const desk = {
  models: [
    'rsa_rss', 'rsa_dir',
    'rs2_rss', 'rs2_dir',
    'm3_lis', 'm3_rsa_rss', 'm3_rsa_dir', // 1.extend
    'm3_lis',  // 2.override
  ],
  mi: {
    m3: {
  ...
  if (c.model.engine === 'prisma') {
    ...
    if (dependentPrismaFiles.length) {
      ...
      if (existPrismaPart.length) {
        ...
        const depSchemaContent = await generateNewSchema(
          c,
          dependentPrismaFiles,
        )


        await generateSchemaFile(
          c.pointFiles.currentFiles.modelFiles.depDartSchemaPrisma,
          [
            '// import by deps ',
            depSchemaContent,
          ]
...
async function generateReferenceDrivers (c: IConfig, h: IDependencyHook[]) {
  ...
  await Promise.all(h.map(obj => new Promise<void>((res, rej) => {
    const code = referenceHookTemp({ path: obj.modulePath })
...
export async function composeSignal(c: IConfig) {
  ...
  await generateReferenceDrivers(c, dependencyDrivers)
}
